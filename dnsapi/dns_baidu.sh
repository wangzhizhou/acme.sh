...
OptionsAlt:
 ...
 Baidu_TTL Resolve ttl seconds, default: 600
...
dns_baidu_add() {
  ...
  case "$_record_domain" in
  _acme-challenge* | acmetest*) ;;
  *)
    _err "Only _acme-challenge* or acmetest* TXT is allowed to add"
    return 1
    ;;
  esac

  if ! _existing_ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "$txtvalue")"; then
    return 1
  fi
  if [ "$_existing_ids" ]; then
    if _baidu_should_debug2; then
      _debug2 "baidu_bcd" "txt exists, skip add: $_record_domain.$_zone_name"
    fi
    return 0
  fi
...
dns_baidu_rm() {
  ...
  _acme-challenge* | acmetest*) ;;
  *)
    _err "Only _acme-challenge* or acmetest* TXT is allowed to delete"
    return 1
    ;;
  esac

  if ! _ids="$(_baidu_find_record_ids "$_zone_name" "$_record_domain" "TXT" "$txtvalue")"; then
    return 1
  fi
  if [ -z "$_ids" ]; then
    if _baidu_should_debug2; then
      _debug2 "baidu_bcd" "no matching txt to delete: $_record_domain.$_zone_name"
    fi
    return 0
  fi
  ...
  if [ "$_rm_cnt" ] && [ "$_rm_cnt" -gt "$_rm_max" ]; then
    _err "Refusing to delete $_rm_cnt records (limit: $_rm_max)"
    return 1
  fi
...
_baidu_prepare_record() {
  ...
  if ! _baidu_get_root "$_fulldomain"; then
    _err "Could not find zone for $_fulldomain"
    return 1
  fi
...
_baidu_get_root() {
  ...
  while true; do
    ...
    if ! _baidu_is_api_error "$response" && (_contains "$response" "\"totalCount\"" || _contains "$response" "\"result\""); then
      ...
      if _baidu_should_debug2; then
        _debug2 "baidu_bcd" "zone matched: $_domain (host: $_sub_domain)"
      fi
      return 0
    fi
...
_baidu_bcd_post() {
  ...
  _auth="$(_baidu_bce_auth "POST" "$_uri" "" "$BAIDU_BCD_HOST" "$_ts" "$_expire" "$_content_type" "$_payload_hash")"
  if [ -z "$_auth" ]; then
    return 1
  fi
  ...
  if _baidu_should_debug2; then
    ...
    _debug2 "baidu_bcd" "POST ${_uri}"
    _debug2 "baidu_bcd.signedHeaders" "$_signed_headers_dbg"
    _debug2 "baidu_bcd.payload_sha256" "$_payload_hash"
    if _baidu_should_debug3; then
      _debug2 "baidu_bcd.payload" "$(_baidu_dbg_trim "$(_baidu_redact_txt "$_payload")")"
    fi
  fi
  response="$(_post "$_payload" "$_url" "" "POST" "$_content_type")"
  _ret="$?"
  if _baidu_should_debug2; then
    _debug2 "baidu_bcd.ret" "$_ret"
    ...
    _debug2 "baidu_bcd.response" "requestId=${_req_id:-"-"} code=${_code:-"-"} message=$(_baidu_dbg_trim "${_msg:-"-"}")"
  fi
  if [ "$_ret" != "0" ]; then
    return 1
  fi
...
${_canonical_headers}"
  ...
  _signature="$(_baidu_hmac_sha256_strkey "$_signing_key" "$_canonical_request")"

  if _baidu_should_debug3; then
    _debug2 "bce_auth"
    _debug2 "auth_prefix" "bce-auth-v1/[ak]/${_ts}/${_expire}/$_signed_headers/[signature]"
    _debug2 "canonical_request_l" "$(printf "%s" "$_canonical_request" | sed -n 'l')"
    _debug2 "signature" "$(printf "%s" "$_signature" | cut -c 1-16)..."
  fi

  printf "%s" "${_auth_prefix}/${_signed_headers}/${_signature}"
}
...
_baidu_should_debug2() {
  # Match acme.sh default logging rules: debug2 prints when any debug2 channel is enabled.
  [ "${LOG_LEVEL:-$DEFAULT_LOG_LEVEL}" -ge "$LOG_LEVEL_2" ] ||
    [ "${SYS_LOG:-$SYSLOG_LEVEL_NONE}" -ge "$SYSLOG_LEVEL_DEBUG_2" ] ||
    [ "${DEBUG:-$DEBUG_LEVEL_NONE}" -ge "$DEBUG_LEVEL_2" ]
}

_baidu_should_debug3() {
  # Match acme.sh default logging rules: debug3 is more verbose and includes signing details.
  [ "${LOG_LEVEL:-$DEFAULT_LOG_LEVEL}" -ge "$LOG_LEVEL_3" ] ||
    [ "${SYS_LOG:-$SYSLOG_LEVEL_NONE}" -ge "$SYSLOG_LEVEL_DEBUG_3" ] ||
    [ "${DEBUG:-$DEBUG_LEVEL_NONE}" -ge "$DEBUG_LEVEL_3" ]
}
...
_baidu_match_record_id() {
  ...
  _rdtype_e="$3"
  _rdata_e="$4"
  if _contains "$_line" "\"recordId\"" &&
    _contains "$_line" "\"rdtype\":\"$_rdtype_e\"" &&
    _contains "$_line" "\"domain\":\"$_domain_e\"" &&
    _contains "$_line" "\"rdata\":\"$_rdata_e\""; then
...
_baidu_hmac_sha256_strkey() {
  _key_str="$1"
  _msg="$2"
  _key_hex="$(printf "%s" "$_key_str" | _hex_dump | tr -d " ")"
  printf "%s" "$_msg" | _hmac sha256 "$_key_hex" hex
}
